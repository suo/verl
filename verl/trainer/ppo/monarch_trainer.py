...
"""
FSDP PPO Trainer with Monarchay-based single controller.
...
from monarch._rust_bindings.monarch_hyperactor.meta.alloc import (
    MastAllocator,
    MastAllocatorConfig,
)
...
# from verl.single_controller.monarch.base import create_colocated_worker_cls
...
def create_mast_proc_mesh(
    job_name: str,
    task_group: str = "script",
    monarch_port: int = 29500,
    ...
    """
    print(
        f"Allocating process mesh: {job_name=}, {task_group=}, {monarch_port=}, {num_hosts=}, {num_gpus=}"
    )
    ...
    # TODO: this should expose a sync API
    async def _allocate_lol(spec):
        return await allocator.allocate(spec)

    allocation = asyncio.run(_allocate_lol(spec))
    hyperactor_mesh = HyProcMesh.allocate_blocking(allocation)
    return ProcMesh(hyperactor_mesh)
...
class ResourcePoolManager:
    ...
    resource_pool_dict: dict[str, MonarchResourcePool] = field(default_factory=dict)
    mast_job: str | None = None

    def create_resource_pool(self):
        """Create Monarch resource pools for distributed training.
        ...
        for resource_pool_name, process_on_nodes in self.resource_pool_spec.items():
            assert (
                len(set(process_on_nodes)) == 1
            ), "Monarch backend currently expects the same number of processes on every noe"
            # TODO make this generic for non-procmesh
            num_procs = process_on_nodes[0]
            if self.mast_job:
                pm = create_mast_proc_mesh(
                    job_name=self.mast_job,
                ...
                if len(process_on_nodes) > 1:
                    raise ValueError(
                        "Multi-node training requested but no mast job provided to run on, please set MONARCH_MAST_JOB"
                    )
                pm = proc_mesh(gpus=num_procs).get()

            # max_colocate_count means the number of WorkerGroups (i.e. processes) in each MonarchResourcePool
            # For FSDP backend, we recommend using max_colocate_count=1 that merge all WorkerGroups into one.
            # For Megatron backend, we recommend using max_colocate_count>1
            # that can utilize different WorkerGroup for differnt models
            resource_pool = MonarchResourcePool(pm=pm)
        ...
        # NOTE: mesh allocation should check this for us.
        # self._check_resource_available()

    def get_resource_pool(self, role: Role) -> MonarchResourcePool:
...
class MonarchPPOTrainer:
    """Distributed PPO trainer using Monarch for scalable reinforcement learning.

    This trainer orchestrates distributed PPO training across multiple nodes and GPUs,
    managing actor rollouts, critic training, and reward computation with Monarch backend.
    Supports various model architectures including FSDP, Megatron, and vLLM integration.
    """

    # TODO: support each role have individual monarch_worker_group_cls,
    ...
    def __init__(
        ...
        monarch_worker_group_cls: type[MonarchWorkerGroup] = MonarchWorkerGroup,
        processor=None,
        reward_fn=None,
        val_reward_fn=None,
        ...
        Args:
            ...
            monarch_worker_group_cls (MonarchWorkerGroup, optional): Class for Ray worker groups. Defaults to MonarchWorkerGroup.
        ...
        self.monarch_worker_group_cls = monarch_worker_group_cls
    ...
    def init_workers(self):
        ...
        Creates:
        1. Monarch resource pools from configuration
        ...
        if self.hybrid_engine:
            ...
            actor_rollout_cls = MonarchClassWithInitArgs(
        ...
        if self.use_critic:
            ...
            critic_cls = MonarchClassWithInitArgs(
        ...
        if self.use_reference_policy:
            ...
            ref_policy_cls = MonarchClassWithInitArgs(
        ...
        if self.use_rm:
            ...
            rm_cls = MonarchClassWithInitArgs(
        ...
        all_wg = {}
        wg_kwargs = {}  # Setting up kwargs for MonarchWorkerGroup
        if (
        ...
        for resource_pool, class_dict in self.resource_pool_to_cls.items():
            ...
            wg_dict = self.monarch_worker_group_cls(
                resource_pool=resource_pool,
                class_with_init_args=worker_dict_cls,
